...
class BehaviorController {
    ...
    void initialize(godot::Node3D* owner, int _owner_id, godot::Vector2 _position);

    //CCNode3D* get_owning_node() const;
    //void set_owning_node(CCNode3D* const direction);

    godot::Vector2 get_input_direction() const;
    void set_input_direction(const godot::Vector2 direction);
